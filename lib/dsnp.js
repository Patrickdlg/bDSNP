...
class DSNP extends EventEmitter{
   ...
   */
  init(){
     console.log('[DSNP] initializing DSNP server over network: %s', this.network);
     this.clients = []
     this.initHttpServer();
   ...
   */
  initHttpServer(){
    console.log('[DSNP] creating DSNP HTTP endpoint');
   ...
   */
  initWebSocketServer(){
    console.log('[DSNP] creating DSNP Web Socket server');
  ...
  onRequest(socket){
    ...
    console.log('[DSNP] received request from: %s', origin)
  ...
  handleMessage(websocket, message){
    if (message.type === 'utf8') {
      console.log('[DSNP] broadcasting utf8 data', message.utf8Data);
      this.clients.forEach(function(previousSocket) {
        if (previousSocket != websocket) previousSocket.sendUTF(message.utf8Data);
    });}
    else if (message.type === 'binary') {
      console.log('[DSNP] broadcasting binary data', message.binaryData);
      this.clients.forEach(function(previousSocket) {
